...
public class NutriProfiler {
	...
	//populates the recommendedNutrientsList using persons’ calculateEnergyRequirement() and calculateNutriRequirement() methods
	static void createNutriProfile(Person person) {
		
		person.recommendedNutrientsList.clear();
		
		//add energy nutrient information to recommendedNutrientList
		person.recommendedNutrientsList.add(new RecommendedNutrient(ENERGY_NUTRIENT_CODE, person.calculateEnergyRequirement()));
		
		int i=0;
		for(NutriEnum nutrient: NutriEnum.values()) //for each constant in NutriEnum
		{
			person.recommendedNutrientsList.add(new RecommendedNutrient(nutrient.getNutrientCode(), person.calculateNutriRequirement()[i]));
			i++;
		}
